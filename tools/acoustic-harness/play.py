...
Usage:
    python play.py --wav <path> --wait-for "Turn complete" --tag DYTTY --timeout 10
    python play.py --wav <path> --wait-for "User said" --tag DYTTY --log session.log
...
Exit codes:
    ...
    2 — Error (file not found, ADB not available, tag not set, etc.)
"""
...
def resolve_tag(flag_value: str | None, required: bool = True) -> str | None:
    ...
    If required=False (play-only mode), returns None when not set.
    """
    if flag_value:
        return flag_value
...
def monitor_logcat(
    ...
    for the [tag] marker + signal. Optionally appends all matched lines
    to a log file.
    """
    tag_marker = f"[{tag}]"
...
def main() -> None:
    ...
    parser.add_argument(
        "--wait-for",
        default=None,
        help="Signal string to watch for in tagged logcat lines (required unless --play-only)",
    )
    parser.add_argument(
        "--tag",
        default=None,
    ...
    parser.add_argument(
        "--log",
        default=None,
        help="Append tagged logcat lines to this file",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--play-only",
        action="store_true",
        help="Play audio without logcat monitoring (for on-device STT like speech_to_text)",
    )
    ...
    if not args.play_only and not args.wait_for:
        print("ERROR: --wait-for is required unless --play-only is used")
        sys.exit(2)

    # Play-only mode: just play audio and exit
    if args.play_only:
        if args.delay > 0:
        ...
        play_wav(args.wav)
        print("DONE: Audio playback complete")
        sys.exit(0)

    # Normal mode: play audio + monitor logcat
    ...
    monitor_thread = threading.Thread(
        ...
        kwargs={"tag": tag, "log_path": args.log},
        daemon=True,
    )
