...
# Auto-increment version in pubspec.yaml (validated parse — see scripts/lib/version.sh)
source "$SCRIPT_DIR/lib/version.sh"
PUBSPEC="$PROJECT_ROOT/pubspec.yaml"
CURRENT_VERSION=$(grep '^version:' "$PUBSPEC" | head -1)
if ! BUMPED=$(parse_and_bump_version "$CURRENT_VERSION" "$BUMP_PATCH"); then
...
# Commit + tag only AFTER a successful build and upload, so a failed distribution
# never leaves a version bump committed or a dangling tag.
git add "$PUBSPEC"
