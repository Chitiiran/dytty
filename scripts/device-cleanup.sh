...
if [[ -n "${DEVICE_TEST_UID:-}" ]]; then
  ...
  echo "WARNING: DEVICE_TEST_UID not set — falling back to auth:export email lookup,"
  echo "         which is unreliable (#206). Prefer setting DEVICE_TEST_UID."
  ...
  TEST_UID=$(firebase auth:export --project "$PROJECT_ID" 2>/dev/null \
    ...
    | cut -d',' -f1 || true)
  if [[ -z "$TEST_UID" ]]; then
    echo "ERROR: Could not find UID for $DEVICE_TEST_EMAIL (auth:export returned nothing — #206)."
    echo "  Set DEVICE_TEST_UID directly to fix this."
    exit 1
  fi
else
  echo "ERROR: Set DEVICE_TEST_UID (preferred) or DEVICE_TEST_EMAIL environment variable."
  exit 1
fi
